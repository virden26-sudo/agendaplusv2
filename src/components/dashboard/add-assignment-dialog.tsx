...
export function AddAssignmentDialog({open, onOpenChange}: AddAssignmentDialogProps) {
    ...
    const handleSave = () => {
        ...
        addAssignment({
            ...
            dueDate: parsedData.dueDate || new Date().toISOString(),
            details: parsedData.details || undefined,
        });
                ...
                {!parsedData ? (
                            ...
                            <FormField
                                ...
                                render={({field}) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Textarea
                                                placeholder="e.g., 'Essay on the Roman Empire due next Friday'"
                                                {...field}
                                                rows={3}
                                            />
                                        </FormControl>
                    ...
                    <div className="space-y-4 pt-4">
                        <div className="space-y-2">
                            ...
                            <Input id="task" defaultValue={parsedData.task}
                                   onChange={(e) => setParsedData({...parsedData, task: e.target.value})}
                                   placeholder="Task"/>
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="course">Course</Label>
                            <Input id="course" defaultValue={parsedData.course || ""}
                                   onChange={(e) => setParsedData({...parsedData, course: e.target.value})}
                                   placeholder="Course"/>
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="dueDate">Due Date</Label>
                            <Input id="dueDate" type="date" defaultValue={parsedData.dueDate || ""}
                                   onChange={(e) => setParsedData({...parsedData, dueDate: e.target.value})}
                                   placeholder="Due Date"/>
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="details">Details</Label>
                            <Textarea id="details" defaultValue={parsedData.details || ""}
