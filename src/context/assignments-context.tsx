...
interface AssignmentsContextType {
    ...
    addAssignment: (assignment: Omit<Assignment, 'id' | 'completed' | 'priority' | 'dueDate'> & {
        dueDate: string | Date
    }) => void;
...
type StoredAssignment = Omit<Assignment, 'dueDate'> & { dueDate: string };
...
export function AssignmentsProvider({children}: { children: ReactNode }) {
    ...
    useEffect(() => {
        const timeoutId = window.setTimeout(() => {
            try {
                ...
                const parsed = JSON.parse(storedAssignments) as StoredAssignment[];
                setAssignments(
                    Array.isArray(parsed)
                        ? parsed.map((assignment) => ({
                            ...assignment,
    ...
    const addAssignment = (assignment: Omit<Assignment, 'id' | 'completed' | 'priority' | 'dueDate'> & {
        dueDate: string | Date
    }) => {
    ...
    return (
        <AssignmentsContext.Provider
            value={{assignments, addAssignment, addMultipleAssignments, toggleAssignment, loading}}>
            {children}
        </AssignmentsContext.Provider>
    );
}
